...
**Current verdict: VERIFIED (HIGH confidence, 2/2).** The current cumulative
checker is
[verify_cumulative.py](../../code/verify_cumulative.py).
Evaluator-visible downloads:
[1,536-row ONNX CSV](../../evidence/cumulative/full_n512.csv),
[independent validation JSON](../../evidence/cumulative/claim1_validation.json),
and
[source audit JSON](../../evidence/cumulative/claim1_source_audit.json).
The exact fixed command is `uv run --locked python repro/src/run_campaign.py`;
its evaluator-visible copy is
