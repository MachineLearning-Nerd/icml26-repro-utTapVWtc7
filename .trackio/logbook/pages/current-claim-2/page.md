...
**Current verdict: VERIFIED (HIGH confidence, 2/2).** The accepted paper-scale
result is APPS memory Spearman ρ=0.926807 at n=512 and KBSS latency ρ=0.535279
at n=512. It supersedes the earlier n=40 default below. The current fixed
command is `uv run --locked python repro/src/run_campaign.py`; current code:
[run_campaign.py](../../code/run_campaign.py)
and
[verify_cumulative.py](../../code/verify_cumulative.py).
Downloads:
[paper-scale summary JSON](../../evidence/cumulative/table3_results.json),
[captured results CSV](../../evidence/cumulative/regresslm_table3_results.csv),
[n=40 row-level historical control CSV](../../evidence/cumulative/apps_n40_canonical.csv),
and
[independent control JSON](../../evidence/cumulative/apps_n40_independent_verification.json).
The paper-scale APPS/KBSS row-level draws were not retained in the judged
repository; that limitation is explicit and no new points are forecast.

## Historical rejected baseline

The n=40 block immediately below was the old weak default. It is preserved
unchanged for provenance and is not the current verification run.
